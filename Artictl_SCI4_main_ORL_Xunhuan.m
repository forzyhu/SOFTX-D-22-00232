...
% pic_address='F:\mat\测试包\口罩测试图\ORL\s';%口罩测试图
...
%训练样本参数设置
Train_num=8;  %用于训练样本的数量，需要更改时，只需要该该值即可
...
for Man_i=1:Man_num
    for Fig_j=1:Fig_num        
        m=strcat(pic_address,int2str(Man_i),'_',int2str(Fig_j),'.bmp');
        %1、读取图片
        img=imread(m);
        [x,y,z]=size(img);
        if(z>1)
...
%         [img]=Artictl_SCI4_Zyh_Fun_CZYW(img,1,1,1,1,1,1,1,1);%    论文4的算法
        
          [img]=Artictl_SCI4_Zyh_Fun_CZYW(img,3.8422,-10.5563,7.5137,-6.1952,-4.3503,-1.2727,-1.0042,-3.8305);%    论文4的算法    (0.2,0.4) YALE推荐 
%         [img]=Artictl_SCI4_Zyh_Fun_CZYW(img,-2.8507,17.3508,24.5187,8.8568,-4.6288,8.6714,20.4349,-1.5919);%    论文4的算法    (0.2,0.4) YALE推荐 
        
        
%         [img]=Artictl_SCI4_Zyh_Fun_CZYW(img,3.5284,2.3740,-4.2648,0.8642,3.0010,-2.4942,15.6236,0.3061);%   论文4的算法    ORL推荐
...
%         [x c img]=Artictl_SCI3_Zyh_Fun_CZYW(img,1);%97%   [x  c img]的结果是98.5%
